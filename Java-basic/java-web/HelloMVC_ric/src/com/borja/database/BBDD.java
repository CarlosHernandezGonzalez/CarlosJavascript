...
public class BBDD {
	...
	public boolean existeEmail(String emailrecibido) {
		...
		for (Usuario unEmail : this.usuarios) {
			...
			if (unEmail.getEmail().equals(emailrecibido)) {//el equals es solo para strings
				existeemail = true;
				break;
			}
		}
	...
	public boolean borrarUsuarioPorId(int idInt) {
		for (Usuario unBorrado : usuarios) {
			if(unBorrado.getId()==idInt) {
				this.usuarios.remove(unBorrado);
				break;
			}
		}
